...
- **Handlers stay in ``tools.py``** and are imported here; this is metadata
  relocation, not a handler rewrite (no git-blame reset, minimal review surface).
  The ``from .tools import …`` below resolves because ``tools.py`` imports this
  module at its *bottom*, after all handler functions are defined — the same
  proven pattern ``_register_generated_tools`` used.
- ``CORE_TOOLS`` values are the same spec ``dict`` objects the legacy literal built
  (key order preserved), so the composed registry is identical key-for-key and
...
# The inline core tools, in EXACT legacy declaration order (do not reorder — the
# advertised tools/list order is pinned by tests/tools/test_modules.py).
CORE_TOOLS = {
    'ListIndexTool': {
